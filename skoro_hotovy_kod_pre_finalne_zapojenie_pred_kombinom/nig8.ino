...
// ─── ON/OFF switch ────────────────────────────────────────────────
// LOW = switch zapnutý (INPUT_PULLUP)
// Logika: ak je switch OFF pri štarte, čakáme kým ho nezapneš.
//         Ak je switch zapnutý a bežíme, vypnutie reštartuje ESP.
...
// ─── frequencyToNote ─────────────────────────────────────────────
// Hľadáme najbližšiu strunu v CENTOCH s limitom ±350 centov.
// Ak je frekvencia príliš ďaleko od všetkých strún (gitara ešte
// naladená v inom ladení), priradíme k najbližšej ale s veľkou
// odchýlkou – ručička jednoducho ukáže maximum.
void frequencyToNote(float frequency) {
  int   bestIndex   = 0;
  float minCentDiff = 9999.0f;
  const float MAX_CENTS = 350.0f;

  // Prvý prechod: hľadáme strunu v rozsahu ±350 centov
  for (int i = 0; i < 6; i++) {
    float centDiff = fabsf(1200.0f * log2f(frequency / activeTuning->frequencies[i]));
    if (centDiff <= MAX_CENTS && centDiff < minCentDiff) {
      minCentDiff = centDiff;
      bestIndex   = i;
    }
  }

  // Ak žiadna struna nie je v rozsahu, vezmeme absolútne najbližšiu
  if (minCentDiff == 9999.0f) {
    for (int i = 0; i < 6; i++) {
      float centDiff = fabsf(1200.0f * log2f(frequency / activeTuning->frequencies[i]));
...
// ─── drawWaitScreen ──────────────────────────────────────────────
// Zobrazí sa keď switch je OFF pri štarte – čaká na zapnutie
void drawWaitScreen() {
  display.clearDisplay();
  display.setTextSize(1);
  display.setTextColor(SH110X_WHITE);
  // "Guitar Tuner" centrovane
  display.setCursor(28, 20);
  display.print("Guitar Tuner");
  // "Switch ON to start" = 18 znakov * 6 = 108px → x = 10
...
void setup() {
  ...
  // ── Čakáme na ON switch pred štartom ──────────────────────────
  // Ak je switch OFF (HIGH pri INPUT_PULLUP), zobrazíme čakaciu
  // obrazovku a blokujeme kým ho nezapneš.
  ...
  // Switch je ON – pokračujeme s normálnym štartom
